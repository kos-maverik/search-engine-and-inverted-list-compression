        ...
        if 'text/html' in response.info().getheader('Content-Type'):
            ...
            if response.code == 200:
                ...
                for link in soup.find_all('a'):
                    ...
                    # 1ST CHANGE
                    # convert url and href to lower-case
                    # so that there are no duplicates in visited_lists
                    url = url.lower()
                    href = link.get('href').lower()

                    # 2ND CHANGE
                    # if the href has '#', delete everything after the symbol
                    # if the href ends with '?', remove it
                    #  now we have even less duplicates
                    href = href.split('#')[0]
                    if href.endswith('?'):
                        href = href[:-1]
